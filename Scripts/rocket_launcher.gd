...
func _ready() -> void:
	...
	if is_player:
		...
		if owner_of_gun.rocket_mag <= 0 and owner_of_gun.rocket_launcher_bullets > 0:
			reload()
...
func shoot(target: Vector2) -> void:
	if fire_timer > 0.0:
		return
	if is_player and owner_of_gun.rocket_mag <= 0:
		return
...
func reload() -> void:
	if is_reloading or owner_of_gun.rocket_launcher_bullets <= 0:
		return
	...
	var amount_to_fill = mag_capacity - owner_of_gun.rocket_mag
	var amount_available = min(amount_to_fill, owner_of_gun.rocket_launcher_bullets)

	owner_of_gun.rocket_mag += amount_available
	owner_of_gun.rocket_launcher_bullets -= amount_available

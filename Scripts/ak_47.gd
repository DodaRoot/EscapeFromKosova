...
const BULLET := preload("res://Scenes/Objects/bullets/akbullet.tscn")
...
@export var enemy_cd := 0.4
@export var camera_offset := 0
@export var camera_zoom := 0.7
...
var audio_stream_player = preload("res://Scenes/Objects/dev/audio_stream_player.tscn")
const AK_47_SHOOT = preload("res://Assets/Audio/Guns/AK47Shoot.mp3")
...
func _ready() -> void:
	...
	if is_player:
		...
		if owner_of_gun.ak_mag <= 0 and owner_of_gun.ak_bullets > 0:
			reload()
...
func shoot(target: Vector2) -> void:
	if fire_timer > 0.0:
		return
	if is_player and owner_of_gun.ak_mag <= 0:
		return
...
func aim_at(target: Vector2) -> void:
	...
	if global_rotation_degrees < 0.0:
		upper_left_arm.rotate(tilt * 0.4)
...
func reload() -> void:
	if is_reloading or owner_of_gun.ak_bullets <= 0:
		return
	...
	var amount_to_fill = mag_capacity - owner_of_gun.ak_mag
	var amount_available = min(amount_to_fill, owner_of_gun.ak_bullets)

	owner_of_gun.ak_mag += amount_available
	owner_of_gun.ak_bullets -= amount_available
...
func shooting_audio():
	var audio_stream = audio_stream_player.instantiate()
	audio.add_child(audio_stream)
	audio_stream.stream = AK_47_SHOOT
	audio_stream.play()

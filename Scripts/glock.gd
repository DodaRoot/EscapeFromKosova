...
const BULLET := preload("res://Scenes/Objects/bullets/pistol_bullet.tscn")
...
@export var mag_capacity := 7
@export var reload_time := 1.5
@export var enemy_cd := 0.8
...
# Audio
@onready var audio_pistolshot: AudioStreamPlayer = $Audio/Audio_PISTOLSHOT
...
func _ready() -> void:
	...
	if is_player:
		...
		if owner_of_gun.glock_mag <= 0:
			owner_of_gun.glock_mag = mag_capacity
...
func shoot(target: Vector2) -> void:
	...
	# Magazine check
	if is_player and owner_of_gun.glock_mag <= 0:
		reload()
		return
	...
	if is_player and owner_of_gun.has_method("play_fire_anim"):
		owner_of_gun.play_fire_anim()

	if is_player:
		owner_of_gun.glock_mag -= 1

	var bullet = BULLET.instantiate()
	bullet.global_position = muzzle.global_position
	bullet.rotation = (target - muzzle.global_position).angle()
	bullet.ownerOfBullet = owner_of_gun
	bullet.bulletDamage = BULLET_DAMAGE
	get_tree().current_scene.add_child(bullet)
	
	audio_pistolshot.play()
...
func _clamp_bones() -> void:
	_clamp_rot(upper_left_arm, -70.0, -30.0)
	_clamp_rot(lower_left_arm, 0.0, 25.0)
	_clamp_rot(left_hand, -60.0, 60.0)
